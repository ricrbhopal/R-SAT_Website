...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (token) {
      // If token exists, but we haven't done the one-time dashboard refresh in this session, do it.
      // This guarantees the dashboard loads fresh after login.
      try {
        const refreshed = sessionStorage.getItem("adminDashboardRefreshed");
        if (!refreshed) {
          // mark refreshed to avoid reload loop, then reload
          sessionStorage.setItem("adminDashboardRefreshed", "true");
          // small timeout to allow current render to finish (optional)
          setTimeout(() => window.location.reload(), 50);
          return; // bail out: we'll reload
        }
      } catch (e) {
  ...
  const handleLoginSuccess = (data) => {
    ...
    // mark that we need a fresh reload to mount dashboard as authenticated
    try {
      sessionStorage.setItem("adminDashboardRefreshed", "true");
    } catch (e) {
      // ignore
    ...
    // ensure state updated (optional) then reload to mount fresh
    setIsAuthenticated(true);
    setShowInlineLogin(false);
    setShowLoginModal(false);

    // Full reload so dashboard re-initializes as authenticated
    setTimeout(() => window.location.reload(), 50);
  };
