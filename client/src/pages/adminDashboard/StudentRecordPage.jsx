...
import { FiUser, FiUsers, FiSearch, FiRefreshCw, FiEye, FiEdit, FiDownload } from "react-icons/fi";
...
export default function StudentRecordPage() {
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((s) => {
      ...
      const fatherName = (s.fatherName || s.parentName || "").toString().toLowerCase();
      return name.includes(q) || email.includes(q) || phone.includes(q) || roll.includes(q) || fatherName.includes(q);
    });
  }, [students, query]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500">
                ...
                {query && (
                  <span className="ml-2">
                    • Filtered: <span className="font-medium text-gray-700">{filteredStudents.length}</span>
                  </span>
                )}
              </p>
            </div>
        ...
        {filteredStudents.length === 0 ? (
          <div className="text-center py-8 sm:py-12">
            ...
            <p className="text-gray-500 text-sm mb-4">
              {query ? "Try changing your search query" : "No student records available"}
            </p>
            <button
              onClick={() => {
                setQuery("");
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">R-SAT ID</th>
                  ...
                  {filteredStudents.map((s, idx) => {
                    ...
                    return (
                      <tr key={id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-700">{cls}</td>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{s.fatherName ?? s.parentName ?? ""}</p>
                        </div>
                      </div>
