...
export default function AdmitCardPage() {
  ...
  useEffect(() => {
    const fetchAdmitCard = async () => {
      ...
      setError("");
      try {
        // TODO: replace this hard-coded id with dynamic id (props/route) if needed
        const id = "69217b4885f965da0518d54b";
        const response = await AdminAPI.getAdmitCardById(id);

        // Normalize response shape (support resp.data or resp.data.data)
    ...
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md border">
      ...
      <div className="mt-8 border-t pt-4">
        ...
        <ul className="list-disc list-inside text-sm text-gray-700">
          ...
          <li>Electronic devices like mobile phones, smartwatches, or calculators are not allowed.</li>
          <li>Follow all instructions provided by the invigilators during the exam.</li>
        </ul>
        <p className="text-sm text-gray-500 mt-4">
          For queries: <a href="mailto:contact@ricr.in" className="text-blue-500">contact@ricr.in</a>
        </p>
      </div>
    </div>
  );
